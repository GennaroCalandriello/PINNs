    ...
    def __init__(self, vtk_dir_pattern="VTK/cylinderFlux_*"):
        ...
        for snap_dir in self.snap_dirs:
            ...
            try:
                ...
                t = None
            if t is not None and t > 0 and t <= total_train_time:
                print(f"Opening snapshot at t={t} (<= {total_train_time})")
                t = t / 1000
...
if __name__ == "__main__":
    ...
    # --- Esempio: patch locale centrata ---
    results, idx_cells = sampler.sample_time_series_patch(
        N=40000, mode="local", center=(10000, 10000), save_path="patch_40k_centro.pkl"
    )
    sampler.plot_patch(idx_cells)

    # --- Esempio: patch globale uniforme ---
    # results, idx_cells = sampler.sample_time_series_patch(
    #     N=40000, mode="uniform", save_path="patch_40k_uniform.pkl"
    # )
    # sampler.plot_patch(idx_cells)

    # --- Esempio: patch globale a griglia ---
    # results, idx_cells = sampler.sample_time_series_patch(
    #     N=100, mode="grid", grid_nx=10, grid_ny=10, save_path="patch_100_grid.pkl"
    # )
    # sampler.plot_patch(idx_cells)
