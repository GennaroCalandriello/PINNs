...
class PINN:
    ...
    def __init__(self, layers, operator, ics_sampler, bcs_sampler, res_sampler, nu, kernel_size):
        ...
        X, _ = res_sampler.sample(int(2e4))
        ...
        self.lam_ruv_val = torch.tensor(1.0).float().to(device) # for residuals
    ...
    def train(self, nIter=10000, batch_size=128, log_NTK=False, update_lam=False):
        ...
        for it in range(nIter):
            ...
            loss_ruv = torch.mean(ruv_pred ** 2)

            # loss = self.lam_r_val * loss_res + self.lam_u_val * loss_bcs + self.lam_ut_val * loss_ics_u_t 
            loss =self.lam_ru_val*loss_ru +self.lam_rv_val*loss_rv+ self.lam_ic_val * loss_ics +self.lam_bc_val * loss_bcs + self.lam_ruv_val * loss_ruv
            ...
            if it % ntk_step == 0:
                ...
                print('Loss: %.3e, Loss_ru: %.3e, loss_rv: %.3e, Loss_bcs: %.3e, Loss_ics: %.3e, Loss_ruv: %.3e' %
                      (loss.item(), loss_ru,loss_rv, loss_bcs, loss_ics, loss_ruv))
                ...
                print(f'lambda_ruv: {self.lam_ruv_val:3e}')
            ...
            if log_NTK:
                ...
                if it % ntk_step == 0:
                    ...
                    res_ntk_uv = res_ntk_u + res_ntk_v
                    ...
                    K_ruv_value = self.compute_ntk(J_ruv, self.D3, J_ruv, self.D3)
                    ...
                    trace_K = np.trace(K_ic_value) + np.trace(K_ru_value) + np.trace(K_rv_value)+ np.trace(K_bc_value)+ np.trace(K_ruv_value)
...
def main():     
    ...
    if train_bool:
        ...
        total_loss.append(model.lam_ruv_log)
