...
def obstacle_mask(x, xc=0.0, yc=0.0, r=0.0):
    xd = x[:, 1:2]
    yd = x[:, 2:3]
    # x, y shape: (batch, 1)
    return ((xd - xc) ** 2 + (yd - yc) ** 2 > r**2).float()


def cylinder_boundary_points(center, radius, N_points, t_val=None):
    """
    center: (xc, yc)
    radius: float
    N_points: quanti punti vuoi campionare sul bordo
    t_val: opzionale, valore/i di tempo (float o array shape (N_points,1))
    """
    ...
    if t_val is None:
        t = np.zeros_like(x)
    elif np.isscalar(t_val):
        t = np.full_like(x, t_val)
    else:
        t = np.array(t_val)
    pts = np.stack([t, x, y], axis=1)  # shape (N_points, 3)
    # Normali uscenti
    nx = np.cos(theta)
    ny = np.sin(theta)
    normals = np.stack([nx, ny], axis=1)  # shape (N_points, 2)
    return pts, normals
...
bc_cyl_sampler = Sampler(
    3, ics_coord, , name="BC_Cylinder"
)
# For the cylinder, we can use a zero boundary condition
