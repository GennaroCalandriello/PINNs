...
class NeuralNet(nn.Module):
    ...
    def forward(self, x):
        ...
        yy = x[:, 2:3]
        # Rete "core"
        out = self.model(x)  # (batch, 3): u, v, p
...
class PINN:
    ...
    def __init__(
        ...
        # Neural Network
        self.nn = NeuralNet(layers).to(device)
...
# )
res_sampler = Sampler(
    3, dom_coord, lambda x: np.random.randn(x.shape[0], 2), name="Residuals"
)
