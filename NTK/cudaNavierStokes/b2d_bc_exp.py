...
class PINN:
    ...
    def net_r(self, t, x, y):
        ...
        residual1, residual2 = self.operator(
            u, v, t, x, y, self.nu, self.sigma_t, self.sigma_x, self.sigma_y
        )
    ...
    def train(self, nIter=1000, batch_size=128, log_NTK=False, update_lam=False):
        ...
        for it in range(nIter):
            ...
            loss_bcs = (
                torch.mean((uv_bc1_batch_tens[:, 0:1] - u_pred_bc1) ** 2)
            ...
            if it % ntk_step == 0:
                ...
                print(
                    "Loss: %.3e, Loss_bcs: %.3e, Loss_ics: %.3e, Loss_ru: %.3e, Loss_rv: %.3e"
            ...
            if log_NTK:
                ...
                if it % ntk_step == 0:
                    ...
                    uv_ics_batch_tens = (
                        torch.tensor(uv_ics_batch, requires_grad=True)
                        .float()
                        .to(device)
                    )
                    ...
                    trace_K = np.trace(K_ic_value) + np.trace(
                        K_bc_value
                    )  # + np.trace(K_ruv_value)
...
def animate_plot_2d():
    ...
    cax = ax.pcolormesh(
        X, Y, U_frames[0], cmap="viridis", shading="auto", vmin=vmin, vmax=vmax
    )
