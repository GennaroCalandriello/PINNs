...
# --- IPERPARAMETRI ---
HIDDEN_CHANNELS = (50, 50, 50, 50, 50)  # can be a tuple/list
C = 100
C_LIST = [512, 256, 64]

BOTTLENECKLATENTSPACE = 64
DROPOUT = 0.0
...
USE_LN = False
EPOCHS = 1500
MODEL_PATH = "model/graph_autoencoder_diffpoolCluster.pth"
LOSS_PATH = "model/loss_history_diffpoolCluster.npy"
KIND_CONV = "GMMConv"
EDGEATTENTIONHIDDEN = 200
USE_SOFT_CLUSTERING = False  # <<< NEW: soft o hard centroids
USE_SOFT_CLUSTERING_INIT = False  # <<< NEW: inizializza soft centroids da z
USE_GUMBEL = True  # <<< NEW: opzionale Gumbel-Softmax

# ==================== Small utils ====================


class SoftCentroidCluster(nn.Module):
    """
    Clustering differenziabile: S = softmax( -||z - c||^2 / tau ) (+ prior alpha)
    - centroids: [C, D] learnable
    - tau: temperatura (learnable opzionale)
    - alpha: prior per-cluster (learnable opzionale)
    """

    def __init__(self, C, D, tau=1.0, learn_tau=True, use_alpha=True):
        super().__init__()
        self.C = C
        self.centroids = nn.Parameter(torch.randn(C, D) * 0.01)
        if learn_tau:
            self.log_tau = nn.Parameter(torch.log(torch.tensor(float(tau))))
        else:
            self.register_buffer("log_tau", torch.log(torch.tensor(float(tau))))
        self.use_alpha = use_alpha
        if use_alpha:
            self.alpha = nn.Parameter(torch.zeros(C))

    @torch.no_grad()
    def init_from(self, feat, how="random"):
        """
        Inizializza i centroidi da feat [N, D] (es. z o data.pos proiettato).
        how='random' (default) prende C righe a caso; puoi metterci FPS/k-means offline se vuoi.
        """
        N, D = feat.shape
        idx = torch.randperm(N, device=feat.device)[: self.C]
        self.centroids.data.copy_(feat[idx])

    def forward(self, feat):
        """
        feat: [N, D]
        return:
          S:  [N, C]  (righe sommano a 1)
          d2: [N, C]  (distanze^2, per diagnostica)
        """
        x2 = (feat**2).sum(dim=1, keepdim=True)  # [N,1]
        c2 = (self.centroids**2).sum(dim=1, keepdim=True).T  # [1,C]
        d2 = x2 + c2 - 2.0 * feat @ self.centroids.T  # [N,C]
        tau = torch.exp(self.log_tau) + 1e-8
        logits = -d2 / tau
        if self.use_alpha:
            logits = logits + self.alpha.unsqueeze(0)
...
class GraphAutoencoderDiffPool(nn.Module):
    ...
    def __init__(
        ...
        use_soft_cluster=USE_SOFT_CLUSTERING,  # <<< soft or hard centroids
        soft_blend=0.5,
    ):
        super().__init__()
        self.C = C
        ...
        self.use_soft_cluster = use_soft_cluster  # <<< NEW
        self.soft_blend = float(soft_blend)  # <<< NEW (0..1)

        # testa "learned" (già tua)
        self.assign_head = nn.Sequential(nn.LayerNorm(H), nn.Linear(H, C))

        # centroidi learnable (differenziabili)
        self.cluster = SoftCentroidCluster(
            C=C, D=H, tau=1.0, learn_tau=True, use_alpha=True
        )  # <<< NEW
        ...
        self.assign_head = nn.Sequential(nn.LayerNorm(H), nn.Linear(H, H))
    ...
    def forward(self, data: Data):
        ...
        # -------- ENCODER --------
        # 1) Estrai feature per clusterizzare (z meglio di x)
        # -------- ENCODER --------
        z, s_feat = self.enc(x, edge_index, edge_attr)  # [N,H], [N,H]

        # (A) assignment "learned" classico (DiffPool)
        # S_logits = F.softmax(self.assign_head(s_feat), dim=-1)  # [N,C]
        S_logits = self.assign_head(s_feat)  # [N,C]

        # (B) assignment differenziabile via centroidi learnable
        if self.use_soft_cluster:
            # metti z qui per avere gradiente end-to-end su encoder e centroidi
            S_soft, _ = self.cluster(z)  # [N,C]   <<< QUI C'È self.cluster(z)
            # blend (0 = solo learned; 1 = solo soft-centroids)
            beta = self.soft_blend
            S = beta * S_soft + (1.0 - beta) * S_logits
            S = S / (S.sum(dim=1, keepdim=True) + 1e-12)
        else:
            S = S_logits
        ...
        return y_hat, {
            ...
            "A_coarse": A_coarse,
            "z_graph": z_graph,
            "w": w,  # <<< NEW: edge weights attn
            "edge_index": edge_index,  # per comodità
        }

    @torch.no_grad()
    def init_soft_clusters(self, data: Data, z_init: torch.Tensor | None = None):
        """
        Inizializzazione centroidi:
          - se passi z_init (ad es. una forward dell'encoder), usa quello
          - altrimenti prova con data.x (o proiezione) come fallback
        """
        if z_init is None:
            base = data.x
            # opz.: puoi proiettare x -> H con una piccola MLP, ma per semplicità random init
            self.cluster.init_from(base, how="random")
        else:
            self.cluster.init_from(z_init, how="random")
...
if __name__ == "__main__":
    ...
    model = GraphAutoencoderDiffPool(
        ...
        skip=True,
        dropout=DROPOUT,
        edge_attn_mode="hard",  # "hard" o "topk"
        edge_topk=2,
        lambda_ent=1e-3,
        lambda_link=1e-3,
    ...
    # init centroidi (puoi fare una mini forward per avere z e usare quello)

    if USE_SOFT_CLUSTERING_INIT:
        with torch.no_grad():
            z_init, _ = model.enc(data.x, data.edge_index, data.edge_attr)
            model.init_soft_clusters(data, z_init=z_init)
