...
# Se in training hai aggiunto cp negli edge_attr
...
def obstacles_to_normalized(obstacles_decoded, norm_obj):
    ...
    for obs in obstacles_decoded:
        out.append(
            circle_to_normalized(obs, norm_obj)
            if obs["type"] == "circle"
            else rect_to_normalized(obs, norm_obj)
        )
    return out
...
# MAIN: animazione
# =========================================================
def animate_patch_time_series_gnn(out_gif=OUT_GIF, model_path=MODEL_PATH):
    # --- Carica dati e normalizzatori ---
    results, _ = dataLoader()
    t0, centers, U, neighbors, edge_index, xynorm, uvnorm = dataNormalizer(results)
    data_ref = createGraphData().to(device)  # solo per leggere in_ch/edge_dim/out_ch
    ...
    # --- Modello ---
    in_ch = data_ref.x.size(-1)
    edge_dim = data_ref.edge_attr.size(-1)
    out_ch = data_ref.y.size(-1)
    try:
    ...
    # Numero di feature che il modello si aspetta in input
    try:
        expected_in = model.in_proj[0].in_features
    except Exception:
        # fallback: prova a leggere da primo Linear trovato
        expected_in = in_ch

    # --- Colleziona il modulo della velocità per tutti i frame (solo nodi fluido) ---
    mag_vals = []
    for t_idx, (t, centers_dec, U_dec, neighbors_i, _) in enumerate(results):
        ...
        # edge features (coerente col training)
        rel = centers_norm[edge_dst] - centers_norm[edge_src]  # [E,2]
        dist = torch.norm(rel, dim=1, keepdim=True)  # [E,1]
        if USE_CP_FEATURE:
            dvel = U_norm[edge_dst] - U_norm[edge_src]
            cp = rel[:, 0:1] * dvel[:, 1:2] - rel[:, 1:2] * dvel[:, 0:1]
            edge_attr = torch.cat([rel, dist, cp], dim=1)  # [E,4]
        else:
            edge_attr = torch.cat([rel, dist], dim=1)  # [E,3]

        # feature geometriche in norm (SDF firmata + indicatore)
        dist_norm, indicator = sdf_features_normalized(
            centers_norm, obstacles_norm
        )  # [N,1],[N,1]

        # normali e componenti (solo se il modello le si aspetta)
        n_hat = estimate_normals_from_sdf(centers_norm, edge_index, dist_norm)  # [N,2]
        t_hat = torch.stack([-n_hat[:, 1], n_hat[:, 0]], dim=1)
        u_n = (U_norm * n_hat).sum(dim=1, keepdim=True)  # [N,1]
        u_t = (U_norm * t_hat).sum(dim=1, keepdim=True)  # [N,1]

        # build x: versione 10 canali e 6 canali
        x_full10 = torch.cat(
            [centers_norm, U_norm, dist_norm, indicator, n_hat, u_n, u_t], dim=1
        )  # [N,10]
        x_base6 = torch.cat(
            [centers_norm, U_norm, dist_norm, indicator], dim=1
        )  # [N,6]
        ...
        if expected_in == 10:
            ...
            raise ValueError(
                f"Il modello si aspetta in_ch={expected_in}, ma sto costruendo solo 6 o 10 canali. Allinea le feature al training."
            )
        ...
        with torch.no_grad():
            uvp_norm = model(data_t)  # [N, out_ch]
    ...
    fig.colorbar(quad, ax=ax, label="|u,v|")
    ax.set_xlabel("x")
    ax.set_ylabel("y")
