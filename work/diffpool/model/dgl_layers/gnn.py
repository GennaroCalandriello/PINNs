...
from diffpool.model.loss import EntropyLoss
from diffpool.model.model_utils import masked_softmax
...
class GraphSageLayer(nn.Module):
    """
    Pure PyTorch version of GraphSAGE layer (supports mean, maxpool, LSTM aggregators).
    """
    ...
    def __init__(
        ...
        # Linear transformation for self features and neighbor features
        self.self_linear = nn.Linear(in_feats, out_feats, bias=bias)
        self.neigh_linear = nn.Linear(in_feats, out_feats, bias=bias)
        if self.use_bn:
    ...
    def forward(self, x, edge_index):
        ...
        """
        x = self.dropout(x)
        src, dst = edge_index  # Both [num_edges]

        if self.aggregator_type == "maxpool":
            # Call your MaxPoolAggregator with src node features grouped by dst
            neigh_aggr = self.aggregator(x, src, dst)
        elif self.aggregator_type == "lstm":
            # Call your LSTMAggregator with src node features grouped by dst
            neigh_aggr = self.aggregator(x, src, dst)
        else:  # "mean" (default)
            # Mean of neighbor features for each node
            neigh_feat = x[src]
            neigh_aggr = torch_scatter.scatter_mean(
                neigh_feat, dst, dim=0, dim_size=x.shape[0]
            )

        out = self.self_linear(x) + self.neigh_linear(neigh_aggr)
        if self.use_bn:
...
class GraphSage(nn.Module):
    ...
    def __init__(
        ...
        # input layer
        self.layers.append(
            GraphSageLayer(in_feats, n_hidden, activation, dropout, aggregator_type)
        )
        # hidden layers
        for _ in range(n_layers - 1):
            self.layers.append(
                GraphSageLayer(n_hidden, n_hidden, activation, dropout, aggregator_type)
            )
        # output layer
        self.layers.append(
