...
# path_pkl = f"patch_{patch_size//1000}k_uniform.pkl"
path_time = "experiments_"
dataSamplerPath = "VTKs/VTKvol/experiments_*"
savingPath = "patches/cavity20k.pkl"
# path_time = "cylinderFlux_"

total_train_time = 1940


def _get_cell_volumes(mesh: pv.DataSet) -> np.ndarray:
    """
    Ritorna un array [n_cells] con i volumi cella.
    Priorità: 'V' (OpenFOAM writeCellVolumes) -> 'Volume' (VTK) -> compute_cell_sizes.
    """
    # 1) campi cellari noti
    if "V" in mesh.cell_data:
        V = mesh.cell_data["V"]
    elif "Volume" in mesh.cell_data:
        V = mesh.cell_data["Volume"]
    else:
        # 2) calcolo via VTK
        mesh = mesh.compute_cell_sizes(length=False, area=False, volume=True)
        if "Volume" not in mesh.cell_data:
            raise RuntimeError(
                "Impossibile ottenere i volumi: nessun campo e compute_cell_sizes non ha creato 'Volume'."
            )
        V = mesh.cell_data["Volume"]
...
def _read_snapshot(snap_dir, total_train_time=total_train_time):
    ...
    # Centri cella e features
    pts = mesh.cell_centers().points  # [n_cells, 3]
    cell_centers = pts[:, [0, 2]].astype(np.float32)  # (x,z)
    U = np.asarray(mesh["U"][:, [0, 2]], dtype=np.float32)  # (Ux,Uz)
    V = _get_cell_volumes(mesh)  # [n_cells]

    # Connettività per celle
    neighbors = [mesh.cell_neighbors(i) for i in range(mesh.n_cells)]
...
class DataSamplerVTK:
    ...
    def sample_local_patch(self, N, center=None, random_time=True):
        ...
        if center is not None:
            dist = np.linalg.norm(
                d["cell_centers"] - np.array(center, dtype=np.float32), axis=1
            )
            start = int(np.argmin(dist))
        else:
            start = int(np.random.randint(self.Ncells))
        visited = set([start])
        to_visit = list(d["neighbors"][start])
        # ♦prendo tutta la mesh
        while len(visited) < N and to_visit:
        ...
        if len(visited) < N:
            extra = set(np.random.choice(self.Ncells, N - len(visited), replace=False))
            visited |= extra
        idx = np.array(list(visited), dtype=np.int64)[:N]
        return self._extract_patch(idx, d, t=d["t"])

    # ---- PATCH UNIFORME (sparsa) ----
    def sample_uniform_patch(self, N, snapshot_idx=0):
        d = self.data_all[snapshot_idx]
        idx = np.random.choice(self.Ncells, N, replace=False).astype(np.int64)
        return self._extract_patch(idx, d, t=d["t"])
    ...
    def sample_grid_patch(self, N, grid_nx=10, grid_ny=10, snapshot_idx=0):
        ...
        for i in range(grid_nx):
            for j in range(grid_ny):
                ...
                idx_in_bin = np.where(mask)[0]
                if len(idx_in_bin) > 0:
                    pick = int(np.random.choice(idx_in_bin))
                    idx.append(pick)
        idx = np.array(idx, dtype=np.int64)[:N]
        return self._extract_patch(idx, d, t=d["t"])
    ...
    def _extract_patch(self, idx, d, t=None):
        ...
        if t is None:
            t = d["t"]
        idx_map = {orig_idx: i for i, orig_idx in enumerate(idx.tolist())}
        neighbors_batch = []
        edge_index = []
        ...
        # ritorno anche idx e V
        return t, cell_centers, U, neighbors_batch, edge_index, idx, V
    ...
    def sample_time_series_patch(
        ...
        mode: "local", "uniform", "grid"
        """
        # Se esiste un pickle già salvato, caricalo
        if save_path is not None and os.path.exists(save_path):
        ...
        if idx_cells is None:
            if mode == "local":
                t0, *_rest, idx_cells, _V = self.sample_local_patch(
                    N, center=center, random_time=False
                )
            elif mode == "uniform":
                t0, *_rest, idx_cells, _V = self.sample_uniform_patch(N)
            elif mode == "grid":
                t0, *_rest, idx_cells, _V = self.sample_grid_patch(N, grid_nx, grid_ny)
            else:
                raise ValueError(f"Unknown patch mode: {mode}")
        ...
        for d in self.data_all:
            ...
            print("times", t)
            neighbors_full = d["neighbors"]
            idx_map = {orig_idx: i for i, orig_idx in enumerate(idx_cells.tolist())}
            neighbors_batch = []
            edge_index = []
            for i, orig_idx in enumerate(idx_cells.tolist()):
                nbs = neighbors_full[orig_idx]
                nbs_in_patch = [idx_map[nb] for nb in nbs if nb in idx_map]
                neighbors_batch.append(nbs_in_patch)
                for nb in nbs:
            ...
            # Includo anche V nella tupla
            results.append((t, cell_centers, U, neighbors_batch, edge_index, V))
...
if __name__ == "__main__":
    ...
    # Esempio accesso al volume del 1° snapshot:
    # t, X, U, nbh, E, V = results[0]
    # print("Volume mean:", V.mean())
