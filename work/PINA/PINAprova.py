...
class SimpleODE(SpatialProblem):
    ...
    output_variables = ["u"]
    spatial_domain = CartesianDomain({"x": [0, 5]})

    domains = {
        "x0": CartesianDomain({"x": 0.0}),
        "D": CartesianDomain({"x": [0, 5]}),
    }
...
problem.discretise_domain(100, "lh", domains=["D"])
...
problem.discretise_domain(n=100, mode="latin", domains=["D"])
...
trainer = Trainer(
    ...
    accelerator="gpu",  # Specifies the computing device ("cpu", "gpu", ...)
...
pts = pinn.problem.spatial_domain.sample(1000, "grid", variables="x")
